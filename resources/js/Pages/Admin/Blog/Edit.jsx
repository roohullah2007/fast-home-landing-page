...
export default function Edit({ auth, blogPost }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        published_at: blogPost.published_at ? blogPost.published_at.slice(0, 16) : '',
        is_featured: blogPost.is_featured || false,
        allow_comments: blogPost.allow_comments !== undefined ? blogPost.allow_comments : true,
    });
    ...
    const handleTitleChange = async (e) => {
        ...
        // Auto-generate slug from title if slug is empty or matches current title
        if (!data.slug || data.slug === blogPost.title?.toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-')) {
            try {
                const response = await fetch('/admin/seo/generate-slug', {
                    method: 'POST',
                    headers: {
    ...
    const updateSeoPreview = async () => {
        try {
            const response = await fetch('/admin/seo/preview', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
            ...
            const result = await response.json();
            setSeoPreview(result.preview);
        } catch (error) {
            console.error('Error updating SEO preview:', error);
        }
    };
    
    useEffect(() => {
        updateSeoPreview();
    }, [data.meta_title, data.meta_description, data.slug, data.excerpt]);
    ...
    return (
        <AdminLayout
            header={<h1 className="text-2xl font-bold text-gray-900">Edit Blog Post</h1>}
        >
