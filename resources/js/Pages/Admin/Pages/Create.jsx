...
export default function CreatePage({ auth }) {
    ...
    const generateRoute = () => {
        if (data.slug) {
            const routeName = data.slug.replace(/-/g, '.');
            setData('route', routeName);
        }
    };
    ...
    return (
        <AdminLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Create New Page</h2>}
        >
